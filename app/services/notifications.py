...
async def send_slack_alert(attack_info: Dict):
    ...
    # Resolve Pylance error: Check if URL is None
    url = settings.SLACK_WEBHOOK_URL
    if not url or not app_state.http_client:
        logging.warning(
            "Slack alert skipped: Webhook URL or HTTP client not ready.")
        return
    ...
    try:
        # url is now guaranteed to be str, not None
        response = await app_state.http_client.post(url, json=message, timeout=settings.LLM_REQUEST_TIMEOUT)
...
async def send_discord_alert(attack_info: Dict):
    ...
    if not url or not app_state.http_client:
        logging.warning(
            "Discord alert skipped: Webhook URL or HTTP client not ready.")
        return
    ...
    try:
        ...
        logging.error(f"Unexpected error sending Discord alert: {e}")
