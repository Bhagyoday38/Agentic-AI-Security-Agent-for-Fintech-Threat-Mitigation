...
async def detect_dos_static(event) -> Optional[Dict]:
    ...
    while app_state.request_timestamps and app_state.request_timestamps[0] < now - 60:
...
async def detect_llm_anomaly(event) -> Optional[Dict]:
    """AI Behavioral Analysis: Optimized to prevent simulation timeouts."""
    if app_state.llm_circuit_state.is_open or not app_state.http_client:
        return None

    # FIX: Non-blocking acquisition. If GPU is busy, skip AI for this event to prevent 15s timeout.
    ...
    async with llm_semaphore:
        try:
            ...
            # Tight 10s timeout to stay well within the simulation script's 15s limit
            res = await app_state.http_client.post(f"{settings.OLLAMA_URL}/api/generate", json=payload, timeout=30.0)
